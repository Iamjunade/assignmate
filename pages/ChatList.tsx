...
export const ChatList = ({ user, onSelect, selectedId }) => {
  ...
  useEffect(() => {
    ...
    // This ensures that when 'last_message' or 'updated_at' changes, the list updates instantly
    const channel = supabase.channel('public:chats')
        .on('postgres_changes', { event: '*', schema: 'public', table: 'chats' }, (payload) => {
            // We could optimize this by checking payload.new.poster_id/writer_id, 
            // but reloading the sorted list is safer and fast enough.
            loadChats();
        })
        .subscribe();
