...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const limit = checkRateLimit(`onboarding:${ip}:${auth.uid}`, 20, 60_000);
  if (!limit.allowed) {
  ...
  const message = normalizeString(req.body?.message, 1000) || "Hi, I'm ready to start.";
    ...
    const chat = model.startChat({
      history: [
        { role: 'user', parts: [{ text: SYSTEM_INSTRUCTION }] },
        {
          ...
          parts: [
            {
              text: "Got it! I'm ready to help the student build their profile. I'll keep it friendly and ask one question at a time.",
            },
          ],
        },
        ...history.map((msg: any) => ({
          role: msg?.role === 'ai' ? 'model' : 'user',
          parts: [{ text: normalizeString(msg?.text, 800) }],
        })),
      ],
    });
